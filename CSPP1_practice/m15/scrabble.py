...
VOWELS_N = 'aeiou'
CONSONANTS_N = 'bcdfghjklmnpqrstvwxyz'
HAND_SIZE = 7
...
def deal_hand(n_in):
    ...
    At least n/3 the letters in the hand should be VOWELS_N.
    ...
    for i in range(num_vowels_n):
        del i
        x_in = VOWELS_N[random.randrange(0, len(VOWELS_N))]
        hand[x_in] = hand.get(x_in, 0) + 1

    for i in range(num_vowels_n, n_in):
        x_in = CONSONANTS_N[random.randrange(0, len(CONSONANTS_N))]
        hand[x_in] = hand.get(x_in, 0) + 1
...
def play_game(word_list):
    ...
    hand = {}
    while True:
        user_input = input("Enter a input n(for new hand)\
or r(to play last hand) or e(exit the game)")
        if user_input == 'n':
